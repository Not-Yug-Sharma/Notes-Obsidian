...
<ol><li>Manual method: <br> On command prompt cd to the directory(location) where you previously cloned the repo. Then just enter "git pull". If there are changes this will automatically apply them to your "Downloaded" repo as well. While using <u>git pull</u> only the changes will be downloaded.</li> <li> Automatic "git pull on startup": <br><ul> Open notepad and paste the following : <br>---------------------------------------------------------------------------------<br>@echo off<br>cd /d "C:\Path\To\Your\Repo"<br>git pull<br>---------------------------------------------------------------------------------<br> And save it as a .bat file (Use note_update.bat for future ease.)</li>
